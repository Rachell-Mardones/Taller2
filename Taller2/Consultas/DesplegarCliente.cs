    ...
    public partial class DesplegarCliente : Form
    {
        ...
        private void VerDatos_Click(object sender, EventArgs e)
        {
            if (cboRutCliente.Text != "")
            {
                ...
                string queryFinal = "select distinct c.Nombre, c.Email, c.Saldo, SUM(dv.TotalAPagar) as 'Monto Total' from cliente as c inner join venta as v on c.Rut = v.ClienteRut inner join detalledeventa as dv on v.ID = dv.VentaID where Rut = '" + query1 + "' ";
                //  and v.fecha >= date_sub(NOW(), INTERVAL 3 month)";
